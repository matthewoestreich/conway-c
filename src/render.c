...
Vector2 viewport_grid_to_world(Viewport* vp, Vector2* grid_pos) {
    return (Vector2){.x = grid_pos->x * vp->cell_size.x,
                     .y = grid_pos->y * vp->cell_size.y};
}
...
void renderer_draw_grid_border(Renderer* renderer, Grid* grid) {
    ...
    for (uint32_t col = 0; col <= renderer->viewport.cols; ++col) {
        float x_pos = (float)col * renderer->viewport.cell_size.x;
        Vector2 start = {.x = renderer->viewport.x + x_pos,
                         .y = renderer->viewport.y};
        Vector2 end = {.x = x_pos, .y = height};
        DrawLineEx(start, end, thickness, line_color);
    }

    for (uint32_t row = 0; row <= renderer->viewport.rows; ++row) {
        float y_pos = (float)row * renderer->viewport.cell_size.y;
        Vector2 start = {.x = 0.0f, .y = renderer->viewport.y + y_pos};
        Vector2 end = {.x = width, .y = y_pos};
        DrawLineEx(start, end, thickness, line_color);
    }
}
